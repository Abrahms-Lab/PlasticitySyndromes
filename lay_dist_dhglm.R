# Code for Johansson et al. 2024 -- lay date/distance DHGLM
...
lay.dist.dhglm <- as.character("
  data {
    ...
    real sam[N_obs];              // sam predictor
    real sst[N_obs];		          // sst predictor
    int sex[N_obs];               // fixed effect for trip
    real center_year[N_obs];      // fixed effect for lay
    real y[N_obs];                // response values
    int mm[N_obs];                // which model component (0=lay date, 1=trip dist)
  }
  ...
  model {
    ...
    for(i in 1:N_obs) {
      if(mm[i] == 0) {
        ...
        mu[i] = beta[1] + beta[5]*center_year[i] + beta[2]*sam[i] + d_yr[1,yrID[i]] + d_peng[1,pengID[i]] + d_peng[2,pengID[i]]*sam[i];
        y[i] ~ normal(mu[i],sd_resid[1]);
      }
      else {
        // Sampling statement for trip dist model
        mu[i] = beta[3] + beta[6]*sex[i] + beta[4]*sst[i]  + d_yr[2,yrID[i]] + d_peng[3,pengID[i]] + d_peng[4,pengID[i]]*sst[i]; 
...
y <- c(scale(obs_lay_data$response), scale(obs_trip_data$response))
sam <- c(obs_lay_data$zscore_SAM, rep(0,nrow(obs_trip_data)))
sst <- c(rep(0,nrow(obs_lay_data)), (obs_trip_data$zscore_sst)) 
yr.orig <- c(obs_lay_data$center_year, obs_trip_data$center_year)
yrID <- as.numeric(as.factor(yr.orig))
N_yr <- length(unique(yrID))
pengID.orig <- c(obs_lay_data$penguinseq,obs_trip_data$penguinseq)
pengID <- as.numeric(as.factor(pengID.orig))
pengID <- as.numeric(factor(pengID, levels = unique(pengID)))
center_year <- c(obs_lay_data$center_year, rep(0,nrow(obs_trip_data)))
sex <- c(rep(0,nrow(obs_lay_data)), obs_trip_data$sex=="M")
K <- 6
N_peng <- length(unique(pengID))
N_obs <- length(y)

# combine to a list
mod.in <- list(mm=mm, y=y, sam=sam, sst=sst, yrID=yrID, N_yr=N_yr, pengID=pengID, K=K, N_peng=N_peng, N_peng2=N_peng,
               N_obs=N_obs, center_year=center_year,  sex=sex)

# Model run values
nchain = 4             # Number of MCMC chains
nwarm = 3000           # Number of warm-up iterations
niter = 6000          # Total number of iterations
nperm <- niter-nwarm   # Number of estimation iterations

# fit model
bimod.data.real <- stan(model_code = lay.dist.dhglm, 
                        data = mod.in, 
...
fixed_efs <- c("center.year","sex")
obs_b1_data<-obs_lay_data
obs_b2_data<-obs_trip_data
...
low <- dem_df[which(dem_df$ID==46877),]
high <- dem_df[which(dem_df$ID==46039),]

ggplot(dem_df, aes(x=b1_Slope.mean,
                   y=b2_Slope.mean)) +
  geom_point(alpha=0.3,stroke=0,size=2) + 
  geom_point(data=low,aes(b1_Slope.mean,b2_Slope.mean),alpha=1,size=3,color="#ccbb44") + 
  geom_point(data=high,aes(b1_Slope.mean,b2_Slope.mean),alpha=1,size=3,color="#228833") + 
  geom_abline(intercept=dem_fit$coefficients[[1]],
              slope=dem_fit$coefficients[[2]]) + 
  theme_classic() +
  theme(legend.position = "none") +
  ylab("Trip Distance Plasticity") + xlab("Laying Date Plasticity")
